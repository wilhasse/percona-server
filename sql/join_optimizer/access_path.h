...
struct AccessPath {
  ...
  uint quick_select_type() {
    uint pq_range_type = PQ_QUICK_SELECT_NONE;
    switch (type) {
      case AccessPath::INDEX_RANGE_SCAN:
        pq_range_type = PQ_INDEX_RANGE_SCAN;
        break;
      case AccessPath::INDEX_MERGE:
        pq_range_type = PQ_INDEX_MERGE;
        break;
      case AccessPath::ROWID_UNION:
        pq_range_type = PQ_ROWID_UNION;
        break;
      case AccessPath::ROWID_INTERSECTION:
        pq_range_type = PQ_ROWID_INTERSECTION;
        break;
      default:
        break;
    }
...
inline AccessPath *pq_range_clone_from(THD *thd, AccessPath *path,
  ...
  switch (path->type) {
    ...
    case AccessPath::ROWID_UNION: {
      ...
      // children的定义：Mem_root_array<AccessPath *> *children;
      Mem_root_array<AccessPath *> new_children_paths(thd->mem_root);
      for (AccessPath *old_path : *param.children) {
        // 对range_scan进行复制
        AccessPath *new_path = pq_range_clone_from(thd, old_path, table);
