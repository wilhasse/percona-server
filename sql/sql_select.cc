...
void QEP_shared_owner::qs_cleanup() {
  ...
  if (old_table() &&
    ...
    Table_ref *const table_ref = old_table()->pos_in_table_list;
    if (table_ref) {
      table_ref->derived_keys_ready = false;
      table_ref->derived_key_list.clear();
    }
    ...
    if ((!group_list.empty() &&
      ...
      */
      ...
      if (!group_list.empty()) {
        ...
        {
          if (add_sorting_to_table(curr_tmp_table - 1, &group_list, true))
            DBUG_RETURN(true);
...
// ======================================
  ...
  */
  ...
  if ((grouped || implicit_grouping) && !m_windowing_steps) {
    ...
    const bool need_distinct = !(qep_tab && qep_tab[0].range_scan() &&
          is_agg_loose_index_scan(qep_tab[0].range_scan()));
  ...
  if (qep_tab && (!group_list.empty() || (!order.empty() && !m_windowing_steps /* [1] */))) {
    ...
    */
    ...
    if (qep_tab &&
      ...
      if (add_sorting_to_table(curr_tmp_table, &order_arg, false))
        return true;
      /*
...
/**
  ...
  @note This function moves tab->select, if any, to filesort-å>select
