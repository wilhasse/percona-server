...
int ha_innobase::pq_leader_range_select_scan_init(uint keyno, void *&pq_ctx,
  ...
  while (!(range_res = mrr_funcs.next(mrr_iter, &mrr_cur_range))) {
    ...
    if (index != nullptr) {
      ...
      if (!heap)
        heap = mem_heap_create(DTUPLE_EST_ALLOC(search_tuple_n_fields), UT_LOCATION_HERE);
    }
...
int ha_innobase::pq_leader_ref_init(uint keyno, void *&pq_ctx,
  ...
  if (index != nullptr) {
    ...
    search_tuple_n_fields = 2 * (index->table->get_n_cols() +
                                 dict_table_get_n_v_cols(index->table));
    if (!heap) heap = mem_heap_create(DTUPLE_EST_ALLOC(search_tuple_n_fields), UT_LOCATION_HERE);
  }
...
int ha_innobase::pq_leader_scan_init(uint keyno, void *&pq_ctx,
  ...
  // range scan
  if (PQ_INDEX_RANGE_SCAN == pq_range_type)
  ...
  if (index == nullptr) {
    if (pq_reader != nullptr) ut::delete_(pq_reader);
    return HA_ERR_GENERIC;
  }
