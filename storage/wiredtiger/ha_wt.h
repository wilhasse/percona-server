...
class wt_share : public Handler_share {
 public:
  THR_LOCK lock;
  wt_share();
  ~wt_share() override { thr_lock_delete(&lock); }
};

/** 
  Class handler
*/
class ha_wt : public handler {
  THR_LOCK_DATA lock;          ///< MySQL lock
  wt_share *share;        ///< Shared lock info
  wt_share *get_share();  ///< Get the share
 ...
 public:
  ha_wt(handlerton *hton, TABLE_SHARE *table_arg);
  ~ha_wt();
 
  const char *table_type() const override { return "wt"; }
  ...
  */
  ...
  /*
    Everything below are methods that we implement in ha_wt.cc.
